...
for i in range(90, 1242):
    previsores.append(base_treinamento_normalizada[i-90:i, 0:2])
...
plt.plot(preco_real_open, color = 'red', label = 'Preço real Abertura')
plt.plot(preco_real_high, color = 'black', label = 'Preço real Alta')

plt.plot(previsoes[: , 0], color = 'blue', label = 'Previsão Abertura')
plt.plot(previsoes[: , 1], color = 'orange', label = 'Previsão Alta')

plt.title('Previsão das ações')
plt.xlabel('Tempo')
